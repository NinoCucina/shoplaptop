<template>
  <v-app :style="{ background: $vuetify.theme.themes.dark.background }">
    <SideBar :drawer="drawer" />
    <RightSideBar />
    <v-container>
      <v-toolbar flat color="rgba(0,0,0,0)">
        <v-btn
          icon
          dark
          class="d-lg-none d-xl-flex"
          @click.stop="drawer = !drawer"
        >
          <v-icon>mdi-menu</v-icon>
        </v-btn>
        <v-spacer></v-spacer>
        <v-text-field
          label="Tìm kiếm "
          class="pt-5 d-none d-sm-flex"
          dark
          filled
          prepend-inner-icon="mdi-magnify"
          solo
          flat
          background-color="transparent"
          rounded
          outlined
        ></v-text-field>
        <v-spacer></v-spacer>
      </v-toolbar>
      <h1 class="white--text">Laptop xịn</h1>
      <p class="grey--text">
        Uy tín tạo lên thương hiệu ở đâu xịn hơn chúng tôi không hoàn tiền
      </p>
      <v-app-bar dark color="rgba(0,0,0,0)" flat class="">
        <v-tabs color="#6F0DFF">
          <v-tabs-slider color="#6F0DFF"></v-tabs-slider>
          <v-tab class="withoutupercase">Tất cả sản phẩm</v-tab>
          <v-tab class="withoutupercase">Asus</v-tab>
          <v-tab class="withoutupercase">MSI</v-tab>
          <v-tab class="withoutupercase">Lenovo</v-tab>
          <v-tab class="withoutupercase">Acer</v-tab>
          <v-tab class="withoutupercase">DELL</v-tab>
          <v-tab class="withoutupercase">MacBook</v-tab>
        </v-tabs>
        <v-spacer></v-spacer>
        <v-btn dark text class="d-none d-sm-flex">
          <v-icon left>mdi-magnify</v-icon>
          Tìm kiếm
        </v-btn>
        <v-btn rounded color="white" class="black--text">
          Danh sách
          <v-icon right>mdi-format-list-bulleted</v-icon>
        </v-btn>
        <v-btn icon>
          <v-icon>mdi-apps</v-icon>
        </v-btn>
      </v-app-bar>
      <v-row class="mt-n14">
        <v-col
          cols="12"
          xs="12"
          sm="6"
          md="4"
          lg="3"
          v-for="(laptop, i) in laptop"
          :key="i"
        >
          <v-card
            class="mx-auto my-12 rounded-xl"
            max-width="374"
            color="#151515"
          >
            <v-img height="170" :src="laptop.image"></v-img>
            <v-toolbar color="transparent" class="mt-n7" flat>
              <v-avatar color="white" rounded class="mr-2">
                <v-img :src="laptop.pic" contain></v-img>
              </v-avatar>
              <v-spacer></v-spacer>
              <v-avatar color="black" rounded class="mr-2" dark>
                <div class="three">
                  <div class="four">
                    <span class="white--text caption">{{ laptop.price }}</span>
                  </div>
                  <div class="five">
                    <span class="green--text caption">InStock</span>
                  </div>
                </div>
              </v-avatar>
            </v-toolbar>
            <v-card-title class="grey--text caption">{{
              laptop.id
            }}</v-card-title>
            <v-card-title class="grey--text text-grey-darken-1 caption mt-n6">{{
              laptop.date
            }}</v-card-title>
            <v-card-text class="white--text text-grey-darken-1 mt-n2">{{
              laptop.marque
            }}</v-card-text>
          </v-card>
        </v-col>
      </v-row>
    </v-container>
  </v-app>
</template>

<script>
import RightSideBar from "../components/RightSideBar.vue";
import SideBar from "../components/SideBar";

export default {
  name: "HomeView",
  data: () => ({
    selection: 1,
    drawer: true,
    laptop: [
      {
        image: "3.jpg",
        pic: "1.png",
        price: "$465",
        id: "ID: CK1911159967352",
        date: "02.12.2021, 3:34 AM",
        marque: "ASUS Zenbook Duo 14 ",
      },
      {
        image: "9.jpg",
        pic: "2.png",
        price: "$714",
        id: "ID: CK1911159967621",
        date: "02.12.2021, 4:12 AM",
        marque: "Macbook M1",
      },
      {
        image: "8.jpg",
        pic: "4.jpg",
        price: "$982",
        id: "ID: CK1911159967532",
        date: "02.12.2021, 6:24 AM",
        marque: "Lenovo Yoga Slim 7",
        
      },
      {
        image: "7.jpg",
        pic: "4.jpg",
        price: "$394",
        id: "ID: CK1911159968663",
        date: "02.12.2021, 7:51 AM",
        marque: "Dell Inspiron 15",
       
      },
      {
        image: "7.jpg",
        pic: "4.jpg",
        price: "$394",
        id: "ID: CK1911159968663",
        date: "02.12.2021, 7:51 AM",
        marque: "Dell Inspiron 15",
       
      },
      {
        image: "7.jpg",
        pic: "4.jpg",
        price: "$394",
        id: "ID: CK1911159968663",
        date: "02.12.2021, 7:51 AM",
        marque: "Dell Inspiron 15",
       
      },
      {
        image: "7.jpg",
        pic: "4.jpg",
        price: "$394",
        id: "ID: CK1911159968663",
        date: "02.12.2021, 7:51 AM",
        marque: "Dell Inspiron 15",
        
      },
      {
        image: "7.jpg",
        pic: "4.jpg",
        price: "$394",
        id: "ID: CK1911159968663",
        date: "02.12.2021, 7:51 AM",
        marque: "Dell Inspiron 15",
        
      },
      {
        image: "7.jpg",
        pic: "4.jpg",
        price: "$394",
        id: "ID: CK1911159968663",
        date: "02.12.2021, 7:51 AM",
        marque: "Dell Inspiron 15",
        
      },
    ],
    
  }),
  components: {
    SideBar,
    RightSideBar,
  },
  methods: {},
};
</script>
<style scoped>
.v-tab.withoutupercase {
  text-transform: none !important;
}
.v-tabs {
  width: 50% !important;
}
.v-btn.withoutupercase {
  text-transform: none !important;
}


.three {
  width: 50px;
  height: 50px;
}

.four {
  width: 50px;
  height: 25px;
  background: black;
}
.five {
  width: 50px;
  height: 25px;
  background: #042a0f;
}
.six {
  width: 50px;
  height: 25px;
  background: #2c2107;
}
</style>
